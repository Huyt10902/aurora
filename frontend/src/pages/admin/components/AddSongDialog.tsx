...
const AddSongDialog = () => {
  ...
  const handleAudioSelect = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setNewSong((prev) => ({
      ...prev,
      duration: "0",
      title: prev.title || getTitleFromFileName(audioFile.name),
    }));
    ...
    try {
      ...
      toast.error("Could not read audio duration from this file");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isReadingAudio || Number(newSong.duration) <= 0) {
        return toast.error("Please wait until audio duration is detected");
      }
    ...
    <Dialog open={songDialogOpen} onOpenChange={setSongDialogOpen}>
      ...
      <DialogContent className="bg-zinc-900 border-zinc-700 max-h-[80vh] overflow-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={isLoading || isReadingAudio}>
            {isReadingAudio
              ? "Reading audio..."
              : isLoading
                ? "Uploading..."
